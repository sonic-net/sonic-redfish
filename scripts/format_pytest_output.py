...
def shorten_test_path(test_path: str, max_width: int = 0) -> str:
    """Extract the human-readable parameter ID from a pytest test path.

    The parameter ID is `<suite>::<case> (<description>)`, generated by
    the JSON-driven runner. We never truncate it: the description carries
    the assertion intent, and truncation hides exactly which test failed.
    """
    # If it's a parameterized test, extract just the parameter ID
    ...
    # max_width <= 0 disables truncation (default). Callers can opt in if needed.
    if max_width <= 0 or len(clean_path) <= max_width:
        return clean_path
...
def main():
    ...
    for line in sys.stdin:
        ...
        path_str = shorten_test_path(test_path)

        # Colorize only the description part in parentheses (Cyan).
        # The description is the outermost (...) at end of line and may itself
        # contain nested parens, so match greedily from the first '(' after
        # the test name through end of string.
